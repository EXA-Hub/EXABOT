...
router.all('/:guildID/mute/:ID', async(req, res) => {
    ...
    if (!targetmember && !targetrole) {
        return res.status(404).json({ message: '👀 | لا أستطيع العثور على العضو أو الرتبة' });
    } else if (targetmember) {
        const mutedPeople = await db.get("muted");
        const mutedMembers = mutedPeople[guildID];
        if (mutedMembers.includes(ID)) return res.status(404).json({ message: 'هذا العضو محظور بالفعل' });
        res.send(mute({ guild }, targetmember));
    } else if (targetrole) {
        if (targetrole.permissions.has('ADD_REACTIONS' || 'SEND_MESSAGES'))
            return res.status(404).json({ message: '🚧 | هذه الرتبة لا تصلح ( الرتبة تمتلك صلاحيات للكتابة والتعليق )' });
