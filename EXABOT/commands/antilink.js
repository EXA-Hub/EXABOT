module.exports = {
    ...
    callback: async({
        ...
        const wrongembed = new MessageEmbed()
            ...
            .addField(`\`${message ?prefix : '/'}antilink info\``, 'رؤية معلومات مانع الدعوات', false)
            .addField(`\`${message ?prefix : '/'}antilink <on/off>\``, 'تشغيل وإيقاف مانع من الدعوات', false)
            .addField(`\`${message ?prefix : '/'}antilink allowed <role>\``, 'تحديد الرتب المسموح لها بنشر الدعوات', false)
            .setTitle(`إستخدام خطأ للأمر: ${message ?prefix : '/'}antilink`);
        ...
        if (args[0]) {
            ...
            const onoffpath = "antilink-on-off";
            if (args[0] === 'on') {
                ...
                const ID = role.id;
                let willsave;
                const allowedpath = "antilink_protection";
                ...
                let roles = 'لا توجد رتب';
                if (allowed[guild.id]) {
                    allowed[guild.id].forEach(roleID => {
                        if (roles = 'لا توجد رتب') return
                        roles = `• <@&${roleID}> •`;
                        roles = roles + `• <@&${roleID}> •`;
                    });
                }
