module.exports = {
    ...
    callback: async({
        ...
        const targetMember = message ? (message.mentions.members.first() ||
            guild.members.cache.get(args[0]) || member) : interaction.options.getMember('شخص');
        const targetUser = targetMember.user;

        if (!targetUser || !targetMember) return '**❌ | لا أستطيع العثور على العضو**';

        const status = targetMember.presence.status;
        const roles = targetMember.roles.cache.map(r => `<@&${r.id}> |`).join(` `);
        ...
        let embed = new MessageEmbed()
            ...
            .addField("تاريخ الإنضمام للسيرفر:", `<t:${Math.floor(member.joinedAt / 1000)}:F>`, true)
            .addField("تاريخ إنشاء الحساب:", `<t:${Math.floor(targetUser.createdTimestamp / 1000)}:d> <t:${Math.floor(user.createdTimestamp / 1000)}:T>`, true)
