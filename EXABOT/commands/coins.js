    ...
    callback: async({
        ...
        if (args[0] === 'add') {
            if (user.id === config.owner) {
                const targetUser = client.users.cache.get(args[2]) ||
            ...
            const topEmbed = new MessageEmbed()
                ...
                .setAuthor(`${guild.name}`, guild.iconURL({ dynamic: true }), config.support.server.invite.link)
            ...
            Object.entries(topObj).forEach(x => {
                ...
                topEmbed.setDescription(topEmbed.description + `\n**\`#${i}\` 🏧*\`${x[1]}\`*🪙 **\n**\`»\`** **\`${topUser ? topUser.tag : "غير معروف"}\`**`);
                i++
            });
            ...
            if (targetUser) {
                if (args[1]) {
                    const gifted = Number(message ? args[1] : interaction.options.getNumber('عدد_العملات'));
                    if (isNaN(gifted)) return '**👀 | أحتاج إلى أرقام**';
                    const userCoins = coins[user.id];
                    if (gifted > userCoins) return '**🤨 | لكنك لا تمتلك هذا القدر من العملات الذهبية**';
                    if (user.id === targetUser.id) return '**😂 | لا تحتاج لهذا حتى**';
                    takeCoins(user.id, gifted);
                    giveCoins(targetUser.id, gifted);
                    targetUser.send(`**${targetUser} 🏧 | مبروك لقد حصلت على ${gifted} عملة ذهبية 🪙 من: ${user}**`).catch(e => { throw e });
