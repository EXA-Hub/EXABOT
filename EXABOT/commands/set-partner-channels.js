module.exports = {
    ...
    callback: async({
        ...
        const nochanneltybeembed = new MessageEmbed()
            ...
            .addField('request', '**القناة اللتي سيرسل البوت طلبات الشراكات فيها**', false)
            .addField('channel', '**القناة اللي سيرسل فيها اعلانات الشراكات**', false)
            .addField('مثال', `**\`${message ? prefix : '/'}spc <channel> request\`**`, false)
            .setFooter(user.tag, user.avatarURL({ dynamic: true }));
        if (!(message ? args[1] : interaction.options.getString('نوع_الغرفة'))) return {
            custom: true,
            embeds: [nochanneltybeembed],
            allowedMentions: { repliedUser: false }
        };
        if ((message ? args[1] : interaction.options.getString('نوع_الغرفة')) !== "request" && (message ? args[1] : interaction.options.getString('نوع_الغرفة')) !== "channel") {
            ...
            const selectedChannel = message ? (guild.channels.cache.get(args[0]) ||
                    guild.channels.cache.find(channel => channel.name.toLowerCase() === args[0]) ||
                    message.mentions.channels.first() || message.channel) :
                interaction.options.getChannel('الغرفة');
            if (!selectedChannel || !selectedChannel.send) return {
                custom: true,
                content: "**👀 | لم أصل للقناة**",
                allowedMentions: { repliedUser: false }
            };
            if (!guild.channels.cache.get(selectedChannel.id)) return {
                custom: true,
            ...
            let path;
            if ((message ? args[1] : interaction.options.getString('نوع_الغرفة')) === "request") path = 'partner_requests';
            if ((message ? args[1] : interaction.options.getString('نوع_الغرفة')) === "channel") path = 'partner_channels';
            let datafile = await db.get(path) || {};
            datafile[guild.id] = selectedChannel.id;
            db.set(path, datafile);
            if ((message ? args[1] : interaction.options.getString('نوع_الغرفة')) === "request") {
                return "**✅ | تم تحديد قناة طلبات الشراكة**";
            } else if ((message ? args[1] : interaction.options.getString('نوع_الغرفة')) === "channel") {
