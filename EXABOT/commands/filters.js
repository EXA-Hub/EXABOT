...
module.exports = {
        ...
        callback: async({
                ...
                if (args[0] === "off" && queue.filters.length > 0) {
                    ...
                    return `${client.emotes.error} | ليس مصفي متاح` + "\n" + `\`\`\`${filtersMap}\`\`\``;
                }
                return `تصفية الصوت الحالية: **${queue.filters.join(", ") || "غير مفعل"+ "\n" + `\`\`\`${filtersMap}\`\`\``}**`;
    },
}