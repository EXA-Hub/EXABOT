module.exports = {
    ...
    callback: async({
        ...
        if (interaction) interaction.reply({ content: `**👍 | نشكرك على إقتراحك**` });
        const config = require('../data/config.js');
        const sugserver = client.guilds.cache.get(config.support.server.id);
        const sugchannel = sugserver.channels.cache.get(config.support.server.suggestion.channel.id);
        ...
        if (!args[0]) {
            ...
            channel.awaitMessages({
                ...
                msg = msg.first();
                sugchannel.send({ embeds: [sugembed.setDescription(msg.content)] }).then(msg => {
                    msg.react("🟢");
                    msg.react("🔴");
                    channel.send({
