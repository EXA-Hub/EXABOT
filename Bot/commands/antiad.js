module.exports = {
    ...
    callback: async({
        ...
        const wrongembed = new MessageEmbed()
            ...
            .addField(`\`${message ?prefix : '/'}antiad info\``, 'رؤية معلومات مانع الدعوات', false)
            .addField(`\`${message ?prefix : '/'}antiad <on/off>\``, 'تشغيل وإيقاف مانع من الدعوات', false)
            .addField(`\`${message ?prefix : '/'}antiad allowed <role>\``, 'تحديد الرتب المسموح لها بنشر الدعوات', false)
        ...
        if (args[0]) {
            ...
            if (args[0] === 'on') {
                ...
                if (!guild.roles.cache.get(role.id)) return "**❌ | لا يمكنك إستعمال رتب من سيرفرات أخرى**";
                const ID = role.id;
                let willsave;
