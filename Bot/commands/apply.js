module.exports = {
    ...
    callback: async({
        ...
        const wrongembed = new Discord.MessageEmbed()
            ...
            .addField(`\`${message ?prefix : '/' + 'apply'} info\``, 'رؤية معلومات التقديم', false)
            .addField(`\`${message ?prefix : '/' + 'apply'} <on/off>\``, 'فتح وإغلاق التقديم', false)
            .addField(`\`${message ?prefix : '/' + 'apply'} role <role>\``, 'تحديد رتبة التقديم', false)
            .addField(`\`${message ?prefix : '/' + 'apply'} channel <channel>\``, 'تحديد قناة التقديم', false)
        ...
        if (args[0]) {
            if (!member.permissions.has("ADMINISTRATOR")) return `× يا صاحبي أنت مش مدير في السيرفر معلش ×`;
            ...
            if (args[0] == 'channel') {
                const channel = message ? (guild.channels.cache.get(args[1]) ||
                    guild.channels.cache.find(channel => channel.name.toLowerCase() === args[1]) ||
                    message.mentions.channels.first() || channel) : interaction.options.getChannel('غرفة');
                if (!channel || channel.type !== "GUILD_TEXT") return "**👀 | لم أصل للقناة**";
                if (!guild.channels.cache.get(channel.id)) return "**❌ | لا يمكنك إستعمال قنوات من سيرفرات أخرى**";
                channel.send(`\\✅ | <#${channel.id}>\n**تم الوصول للقناة وجاري تدوين المعلومات**`).then(msg => {
                ...
                channel.send(`\\✅ | <@&${role.id}>\n**تم الوصول للرتبة وجاري تدوين المعلومات**`).then(msg => {
                ...
                saveData(args[0], onoffpath).then(channel.send(`**✅ | تم فتح التقديم**`));
                return '**👍 | تم تنفيذ العملية**';
            } else if (args[0] == 'off') {
                saveData(args[0], onoffpath).then(channel.send(`**❎ | تم إغلاق التقديم**`));
                return '**👍 | تم تنفيذ العملية**';
            } else {
            ...
            if (!onOffDataFile || !onOffDataFile[guild.id] || onOffDataFile[guild.id] == 'off') return `**❌ | التقديم مغلق حاليا**`;
            if (!rolesDataFile[guild.id]) return "**⁉ | يرجى تحديد رتبة التقديم**\n\`" + message ? prefix : '/' + 'apply' + " role <role>\`"
            if (!channelsDataFile[guild.id]) return "**يرجى تحديد قناة إستقبال طلبات التقديم**\n\`" + message ? prefix : '/' + 'apply' + " channel <channel>\`";
            if (member.roles.cache.get(rolesDataFile[guild.id])) return "**🧐 | أنت بالفعل ضمن طاقم الإدارة**";
                    ...
                    m.edit({ content: `${user}, \`ما هو اسمك?\`` }).then((m) => {
                        m.channel.awaitMessages({ filter, max: 1, time: 60 * 1000, errors: ['الزمن'] }).then((m1) => {
                            ...
                            m.edit(`${user} \`2\``).then((m) => {
                                ...
                                m.channel.awaitMessages({ filter, max: 1, time: 60 * 1000, errors: ['الزمن'] }).then((m2) => {
                                    m2 = m2.first();
                                    var age = m2.content;
                                    m2.delete();
                                    channel.send({ content: `${user} \`3\`` }).then((m) => {
                                        ...
                                        m.channel.awaitMessages({ filter, max: 1, time: 60 * 1000, errors: ['الزمن'] }).then((m3) => {
                                            m3 = m3.first();
                                            var ask = m3.content;
                                            m3.delete();
                                            channel.send({ content: `${user} \`4\`` }).then((m) => {
                                                m.edit({ content: `${user}, \`لماذا تريد أن تصبح ضمن طاقم الإدارة?\`` });
                                                setTimeout(() => {
                                                    m.delete();
                                                }, 10000);
                                                m.channel.awaitMessages({ filter, max: 1, time: 60 * 1000, errors: ['الزمن'] }).then((m4) => {
                                                    m4 = m4.first();
                                                    var ask2 = m4.content;
                                                    m4.delete();
                                                    channel.send({ content: `${user} \`5\`` }).then((m) => {
                                                        m.edit({ content: `${user}, \`كم مدة تفاعلك?\`` });
                                                        m.channel.awaitMessages({ filter, max: 1, time: 60 * 1000, errors: ['الزمن'] }).then((m5) => {
                                                            m5 = m5.first();
                                                            var ask3 = m5.content;
                                                            m5.delete();
                                                            m.edit({ content: `${user}, ***يتم إرسال البيانات***` }).then((mtime) => {
                                                                setTimeout(() => {
                                                                    let embed = new Discord.MessageEmbed()
                                                                        ...
                                                                        .addField('> __متى تم إنشاء حسابك: __', `<t:${Math.floor(user.createdTimestamp / 1000)}:R>`, true);
                                                                    ...
                                                                    let donningrow = new Discord.MessageActionRow()
                                                                        .addComponents(donebtn, undonebtn);
                                                                    channel.send({ embeds: [embed], components: [donningrow] }).then(() => {
                                                                        channel.send({ content: '**✅ | تم إرسال طلبك بنجاح**' });
                                                                    });
                                                                }, 2500);
                                                                setTimeout(() => {
                                                                    mtime.delete();
        ...
        client.on('interactionCreate', interaction => {
            ...
            if (!interaction.member.permissions.has("ADMINISTRATOR" || "MANAGE_ROLES")) return interaction.reply.send("❌ | يجب أن تملك صلاحية `ADMINISTRATOR` أو `MANAGE_ROLES`", true);
            if (interaction.customId == `undone${user.id}`) {
                interaction.message.delete().then(interaction.channel.send(`**❌ | ${user} تم رفض تقديمك**`));
            }
