module.exports = {
    ...
    callback: async({
        ...
        if (interaction) interaction.reply({ content: `**👍 | نشكرك على التبليغ**` });
        const config = require('../data/config.js');
        const reportserver = client.guilds.cache.get(config.support.server.id);
        const reportchannel = reportserver.channels.cache.get(config.support.server.report.channel.id);
        const { MessageEmbed, MessageButton, MessageActionRow } = require("discord.js");
        const invitebtn = new MessageButton()
            .setURL(config.support.server.invite.link)
            .setStyle("LINK")
            .setLabel('روم الإقتراحات');
        const reportinvite = await channel.createInvite({
        ...
        let reportembed = new MessageEmbed()
            ...
            .setAuthor(member.user.tag, member.user.displayAvatarURL({ dynamic: true }), reportinvite.url)
        ...
        if (!args[0]) {
            channel.send({ content: '> **\\⭕ يرجى إعطاء رسالة إخبارية بالخطأ \\⭕**' });
            ...
            channel.awaitMessages({
                ...
                msg = msg.first();
                reportchannel.send({ embeds: [reportembed.setDescription(msg.content)] }).then(msg => {
                    msg.react("🟢");
                    msg.react("🔴");
                    channel.send({
            ...
            reportchannel.send({
                embeds: [reportembed.setDescription(message ? args.join(" ") :
                    interaction.options.getString('الإبلاغ'))]
            }).then(msg => {
                msg.react("🟢");
                msg.react("🔴");
