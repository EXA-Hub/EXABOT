module.exports = {
    ...
    callback: async({
        ...
        const partnerinfoembed = new MessageEmbed()
            ...
            .setAuthor(client.user.username, client.user.avatarURL({ dynamic: true }), config.support.server.invite.link)
            .setDescription(`<#${request[guild.id]}> **روم طلبات الشراكة**\n<#${channels[guild.id]}> **روم إعلانات ونشر الشراكة**\n<@&${role[guild.id]}> **رتبة الشركاء**\n**رسالة الشراكة:**\n\`\`\`${partner_message[guild.id]}\`\`\``)
            .setTimestamp()
